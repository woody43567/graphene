...
import { parseNodeRef, resolveNodeRef, resolveUpsertRef, resolveWriteTarget, rewriteRepoRelative, } from "./scope.js";
const TOOLS = [
    {
        name: "status",
    ...
    {
        name: "read",
        description: "Read the context graph. No arguments returns the full index (all node names, types, summaries). With a name argument, returns the full node including outgoing edges, incoming dependents with neighbor summaries, and observations. In multi-repo sessions, index and node results include a repo field, and name accepts repo:name to disambiguate.",
        inputSchema: {
            type: "object",
            properties: {
                name: {
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                offset: {
                    type: "number",
                    description: "Skip this many results (for pagination). Default 0. Use the 'total' and 'has_more' fields in the response to know if more results exist.",
                },
            },
            required: ["query"],
        },
    ...
    {
        name: "link",
        description: "Create an edge between two nodes. Bidirectional types (related_to, mirrors) automatically create edges in both directions. Idempotent: re-linking updates the reason. Names accept repo:name in multi-repo sessions; from and to must resolve to the same repo (cross-repo edges are not supported).",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "unlink",
        description: "Remove an edge between two nodes. Without type, removes all edges between them. Names accept repo:name in multi-repo sessions.",
        inputSchema: {
            type: "object",
            properties: {
                from: { type: "string", description: "Source node name" },
    ...
    {
        name: "globals_export",
        description: "Export all global facts to a portable markdown bundle at path, for moving user-level facts between machines.",
        inputSchema: {
            type: "object",
            properties: {
                path: {
    ...
    {
        name: "globals_import",
        description: "Import global facts from a portable markdown bundle at path, merging by category+subject. Facts already " +
    ...
    {
        name: "project_read",
        description: "Read repo-scoped facts (conventions, context, decisions specific to this project). No arguments returns all project facts. In multi-repo sessions, repo is required.",
        inputSchema: {
            type: "object",
            properties: {
                ...
                repo: {
                    type: "string",
                    description: "Repo name. Optional in single-repo sessions (must match if given); required in multi-repo sessions.",
                },
            },
        },
    },
    {
        name: "project_write",
        description: "Write a repo-scoped fact. One fact per category+subject pair; writing to an existing pair replaces the content. Use for project-specific conventions, decisions, and context that don't belong on a node. In multi-repo sessions, repo is required.",
        inputSchema: {
            type: "object",
            properties: {
                category: { type: "string", description: "Fact category" },
    ...
    {
        name: "project_delete",
        description: "Remove a repo-scoped fact by category and subject. In multi-repo sessions, repo is required.",
        inputSchema: {
            type: "object",
            properties: {
                category: { type: "string", description: "Fact category" },
...
export function createServer(ctx) {
    ...
    server.oninitialized = () => {
        ...
        if (ctx.scopes.length === 1)
            stripGrapheneBlock(ctx.scopes[0].root);
    };
...
// the scope.
function resolveProjectRoot(tool, scopes, args) {
    ...
    if (repo === undefined) {
        throw new Error(`${tool} requires a "repo" argument in a multi-repo session. In scope: ${scopeNames(scopes)}.`);
    }
...
function checkSameScope(fromRef, toRef, scopes) {
    const fromR = resolveNodeRef(fromRef, scopes);
    const toR = resolveNodeRef(toRef, scopes);
    if (fromR.scope.root !== toR.scope.root) {
        throw new Error(`Cross-repo edges are not supported: "${fromRef}" is in ${fromR.scope.name}, ` +
...
function dispatchGlobalsExport(globalDir, args) {
    const path = args.path;
    if (!path)
        throw new Error("path is required");
    return exportGlobals(globalDir, path);
}
function dispatchGlobalsImport(globalDir, args) {
    const path = args.path;
    if (!path)
        throw new Error("path is required");
...
export function dispatch(ctx, tool, args) {
    switch (tool) {
        case "global_read":
        ...
        default: {
            ...
            if (scopes.length === 1)
                return dispatchSingle(scopes[0].root, ctx.globalDir, tool, args);
...
function dispatchSingle(repoRoot, globalDir, tool, args) {
    switch (tool) {
        case "status":
...
function dispatchMulti(scopes, globalDir, tool, args) {
    switch (tool) {
        case "status":
...
function routeStatus(scopes, globalDir) {
...
function routeRead(scopes, args) {
    const name = args.name;
    if (!name) {
        const nodes = scopes.flatMap((scope) => {
            const result = handleRead(scope.root, {});
    ...
    const result = handleRead(resolved.scope.root, { ...args, name: resolved.name });
    return { repo: resolved.scope.name, ...result };
}
function routeSearch(scopes, globalDir, args) {
    const emptyDir = emptyStoreDir();
...
function routeLearn(scopes, args) {
...
function routeRemoveObservation(scopes, args) {
    const nodeName = args.node_name;
    if (!nodeName)
        throw new Error("node_name is required");
    const { scope, name } = resolveNodeRef(nodeName, scopes);
...
function routeDeleteNode(scopes, args) {
    const name = args.name;
    if (!name)
...
function routeLink(scopes, args) {
...
function routeUnlink(scopes, args) {
    const from = args.from;
    const to = args.to;
    if (!from || !to)
...
function routeUpsertNode(scopes, args) {
    ...
    const covers = Array.isArray(normalized.covers) ? normalized.covers : [];
    const entryPoints = Array.isArray(normalized.entry_points) ? normalized.entry_points : [];
    const scope = resolveWriteTarget(scopes, routed.name, cwd, [...covers, ...entryPoints]);
    const rewritten = { ...normalized, name: routed.name };
    if (covers.length > 0)
        rewritten.covers = rewriteRepoRelative(scope, cwd, covers);
    if (entryPoints.length > 0)
        rewritten.entry_points = rewriteRepoRelative(scope, cwd, entryPoints);
...
function routeBatch(scopes, args) {
    const unknownKeys = Object.keys(args).filter((k) => !BATCH_KEYS.has(k));
    if (unknownKeys.length > 0) {
        throw new Error(`Unknown keys: ${unknownKeys.join(", ")}. batch accepts three top-level arrays: nodes, edges, observations.`);
    }
    const params = args;
    ...
    function resolveItemRef(ref) {
        ...
        if (placedScope)
            return { scope: placedScope, name: parsed.name };
    ...
    for (const rawNode of params.nodes ?? []) {
        ...
        if ("scope" in parsed) {
            ...
            if ("scope" in routed) {
                ...
                const entryPoints = Array.isArray(node.entry_points) ? node.entry_points : [];
                scope = resolveWriteTarget(scopes, routed.name, cwd, [...covers, ...entryPoints]);
                if (covers.length > 0)
                    node.covers = rewriteRepoRelative(scope, cwd, covers);
                if (entryPoints.length > 0)
                    node.entry_points = rewriteRepoRelative(scope, cwd, entryPoints);
            }
        }
    ...
    for (const edge of params.edges ?? []) {
        ...
        if (fromR.scope.root !== toR.scope.root) {
            throw new Error(`Cross-repo edges are not supported: "${from}" is in ${fromR.scope.name}, ` +
